...
#------------------------------------------------------------------------------------------------------
	...
	# We call the super init
		...
		self.time_file=open('server'+str(self.vessel_id)+'.txt', 'a')
		self.cmpt_POST_entryes_vessel=0
...
#------------------------------------------------------------------------------------------------------
	...
	def sort_store(self):
		...
		if len(store_seq_num)==len(self.store):
...
#------------------------------------------------------------------------------------------------------
	...
	def propagate_value_to_vessels(self, path, action, key, value):
		retry_list = set() # we will store the vessels without success
		...
		for vessel in self.vessels:
			...
			if vessel != ("10.1.0.%s" % self.vessel_id):
				...
				if not success:
					retry_list.add(vessel)

		# for the vessels who failed, we retry
		try_count = 0
		while len(retry_list)>0 and try_count < 15:
			time.sleep(1)
			print("Retrying..." + str(len(retry_list)))
			try_count += 1
			to_remove = []
			for vessel in retry_list:
				success = self.contact_vessel(vessel, path, action, key, value)
				if success : to_remove.append(vessel)
			for vessel in to_remove:
				retry_list.remove(vessel)
		if len(retry_list)>0:
...
#------------------------------------------------------------------------------------------------------
	def do_POST(self):
		...
		if valid_path_entries:
			self.set_HTTP_headers(200)
			#thread = Thread(target=self.do_POST_entries,args=(parsed) )
...
#------------------------------------------------------------------------------------------------------
	...
	def do_POST_entries(self, parsed):
		...
		if isValid:
			...
			thread = Thread(target=self.do_POST_entries_vessel,args=(action,key,value,propagate) )
			# We kill the process if we kill the server
			thread.daemon = True
			# We start the thread
			thread.start() 
	...
	def do_POST_entries_vessel(self, action, key, value, propagate):
		self.server.cmpt_POST_entryes_vessel+=1
		print("________________________"+str(self.server.cmpt_POST_entryes_vessel))
		if not propagate:
		...
		self.server.time_file.write(str(end - start)+"\n") 
		if propagate:
